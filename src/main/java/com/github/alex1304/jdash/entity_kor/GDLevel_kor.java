...
public final class GDLevel extends AbstractGDEntity {
	...
	public GDLevel(long id, String name, long creatorID, String description, Difficulty difficulty,
			...
			Length length, Supplier<Mono<GDSong>> song, int coinCount, boolean hasCoinsVerified, int levelVersion,
			int gameVersion, int objectCount, boolean isDemon, boolean isAuto, long originalLevelID, int requestedStars,
	...
	/**
	 * Gets the song that this level uses. (이 레벨에서 사용하는 노래를 가져오십시오.)
	 * 
	 ...
	 *         (노래를 틀어주는 모노로 반환한다. 노래를 Geometry Dash에서 사용할 수 없는 경우, @link
	 *         SongNotAllowedForUseException를 틀어준다.)
	 * 
	 */
	 ...
	 * upload an d last updated dates, and the copy passcode. (레벨의 이진 데이터, 업로드 날짜 및
	 * 최종 업데이트 날짜 및 사본 암호를 포함하는 수준 다운로드 데이터를 가져옵니다.)
	 * 
	 * 
	 * @return a Mono emitting the download data for this level. (이 레벨의 다운로드 데이터를
	 *         내보내는 모노로 반환한다.)
	 */
	...
	/**
	 * Performs a request to know whether the level has been deleted. (수준이 삭제되었는지 여부를
	 * 확인하는 요청 수행)
	 * 
	 * @return true if the level is still on the GD servers, false if deleted. (true가
	 *         GD 서버에 있는 경우 true, 삭제된 경우 false 로 반환한다.)
	 */
	public Mono<Boolean> isDeleted() {
		return refresher.get().map(__ -> false).onErrorReturn(MissingAccessException.class, true);
	}

	/**
	 * Returns a new GDLevel that is a refreshed version of this one. (이 새로 고친 버전 인 새
	 * GDLevel을 반환합니다.)
	 * 
	 * @return a new GDLevel (새 GDLevel을 반환한다.)
	 */
	...
	public String toString() {
		return "GDLevel [name=" + name + ", creatorID=" + creatorID + ", creatorName=" + creatorName + ", description="
				...
				+ ", likes=" + likes + ", length=" + length + ", coinCount=" + coinCount + ", hasCoinsVerified="
				+ hasCoinsVerified + ", levelVersion=" + levelVersion + ", gameVersion=" + gameVersion
				+ ", objectCount=" + objectCount + ", isDemon=" + isDemon + ", isAuto=" + isAuto + ", originalLevelID="
				+ originalLevelID + ", requestedStars=" + requestedStars + "]";
	}
}
