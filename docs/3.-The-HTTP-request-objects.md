...
```Java
GDLevelSearchHttpRequest request = new GDLevelSearchHttpRequest(Constants.LEVEL_SEARCH_TYPE_FEATURED, 0);
```
...
### `GDSongInfoHttpRequest`

Fetches metadata for a Newgrounds song
...
```Java
GDSongInfoHttpRequest request = new GDSongInfoHttpRequest(123456);
```

**Response object:** `GDSong`

An object containing metadata of the requested song

```Java
GDSong song = client.fetch(request);
System.out.println(song);
```

### `GDTimelyLevelHttpRequest`

Fetches a "timely" level, aka Daily level or Weekly demon.

**Constructor parameters:**

- weekly (`boolean`), whether to fetch Daily level (false) or Weekly demon (true)

```Java
GDTimelyLevelHttpRequest request = new GDTimelyLevelHttpRequest(false);
```

**Response object:** `GDTimelyLevel`

Similar to `GDLevel` but also provides info on the Daily/Weekly number and the cooldown in seconds until the next one.


```Java
GDTimelyLevel tl = client.fetch(request);
System.out.println("Daily level: " + tl);
```
